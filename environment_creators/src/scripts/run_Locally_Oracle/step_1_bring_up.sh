...
docker-compose -f ../../iac/docker-compose/use_oracle.yml up -d

figlet -w 160 -f small "Set Oracle Secrets in Vault"
...
vault kv put -address=$VAULT_ADDRESS ENVIRONMENTS/$ENVIRONMENT/Oracle address=oracle_container port=1521 user=system password=Oradoc_db1 instance=ORCLCDB.localdomain status=created
...
vault kv put -address=$VAULT_ADDRESS ENVIRONMENTS/$ENVIRONMENT/Oracle address=oracle_container port=1521 user=system password=Oradoc_db1 instance=ORCLCDB.localdomain status=running

#figlet -w 160 -f small "Create Database"
#cd ../../../..
#python remote_database_updater.py -f localhost_oracle.csv
#cd -
#
...
#while true ; do
#  echo "doing test"
#  curl http://localhost:8080/zipster -d '{"radius":"2.0", "zipcode":"07440"}'
